const Spotify = {
  ...
  async search(term, token) {
    let response = await (
      await this.fetchSpotify(`search?type=track&q=${term}`, token)
    ).json()
    if (!response) return []
  ...
  async getPlaylists(user, token) {
    let response = await (
      await this.fetchSpotify(`users/${user.id}/playlists`, token)
    ).json()
    if (!response) return []
  ...
  async get50LikedTracks(offset, token) {
    let response = await (
      await this.fetchSpotify(`me/tracks?offset=${offset}&limit=50`, token)
    ).json()
    if (!response) return []
    return response.items.map((item) => this.getTrackDetails(item))
  },
  ...
  async getGenreByArtist(artistId, token) {
    let response = await (
      await this.fetchSpotify(`artists/${artistId}`, token)
    ).json()
    if (!response) return []
    ...
    await Promise.all(
      tracks.map(async (track) => {
        ...
        trackGenres.forEach((genre) => {
          if (!playlistGenres.has(genre)) playlistGenres.set(genre, 1)
          else playlistGenres.set(genre, playlistGenres.get(genre) + 1)
        })
      })
    )
